	...
	$.ajax({
		...
		success : (r) => {
			...
			if( r.mid == null ){	// 2. 로그인 안했으면 
				...
				html += `<a href="/jspweb/Ex/Member/Signup.jsp">회원가입</a>`;
				html += `<a href="/jspweb/Ex/Member/login.jsp">로그인</a>`;
				...
				html += 
						...
						<div class="dropdown"> <!-- bs : 드롭다운 -->
							<button class="hpimghtn" type="button" data-bs-toggle="dropdown">
								<img src="/jspweb/Ex/Member/pimg/${ r.mimg == null ? 'default.webp' : r.mimg }" class="hpimg">
							</button>
							<ul class="dropdown-menu">	<!-- 드롭다운시 표기되는 구역 -->
								<li> <a class="dropdown-item" href="/jspweb/Ex/Member/info.jsp"> 내프로필 </a></li>
								<li> <a class="dropdown-item" href="#"> 친구목록 </a></li>
								<li> <a class="dropdown-item" href="/jspweb/Ex/Member/logout.jsp"> 로그아웃 </a></li>
							</ul>
						</div>	<!-- 드롭다운 end  -->
						${r.mid}님
						<a href="#"> 쪽지함 </a>
				...
				if( r.mid == 'admin'){ // 관리자이면 
					html += `<a href="/jspweb/Ex/Admin/Info.jsp">관리자</a>`
				}
