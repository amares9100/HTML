package Day14;


import java.sql.Connection;
import java.sql.DriverManager;
import java.sql.PreparedStatement;

/*
	
	JDBC : 자바와 DB 연결 드라이브
		- 해당 DBMS마다 라이브러리 [.JAR]파일 필요
		
	- 라이브러리 추가
		1. 프로젝트 오른쪽 클릭 -> build path
		2. 라이브러리 탭에서 javaSE 버전 1.8로 변경
		3. javaSE 1.8 ->> add ->> mysql-connector-j-8.0.32.jar 추가
		
	-Connection : DB연결 인터페이스. 다양한 객체를 제공
		- DriverManager.getConnection(db주소 , 계정 , 비밀번호);
			- 서버주소 : jdbc:mysql//ip주소 : 포트번호/db주소 , 계정 , 비밀번호
			- 로컬pc 기준 : jdbc:mysql://localhost:3306/db명
*/

public class Ex4_DB연동 {
	public static void main(String[] args) {
		try { 
			
		Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/java", "root" , "1234");
		System.out.println("DB 연동 성공");
		
		
		// 테이블 생성(테이블 생성 sql구문을 문자열 변수에 저장)
		String sql = "create table member(mmo int , mid varchar(20) , mpw varchar(20))";
		
		// 3 DML DDL 조작 인터페이스
		PreparedStatement ps = con.prepareStatement(sql);
		
		// 4 sql 실행
		ps.execute();
		
		
		}
		catch(Exception e) {
			System.out.println("DB 연동 실패 사유 : "+ e);
		}
	}
}












